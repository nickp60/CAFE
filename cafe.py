...
citation = 'If you use GEMINI, then please cite \"Jani, Mehul, Kalai Mathee, and Rajeev K. Azad. \"Identification of novel genomic islands in Liverpool epidemic strain of Pseudomonas aeruginosa using segmentation and clustering.\" Frontiers in microbiology 7 (2016)\".\nThank you\n'
info = 'GEMINI is a genome mining tool, gemini 1.0 developed by Mehul Jani, University of North Texas'
...
def get_arguments():
    ...
    parser.add_argument(
        "-gbk", help="genome file in GenBank (full) format")
    parser.add_argument(
        "-output", help="Output filename")
    parser.add_argument(
        "-cite",
        help="show publication reference and exit",
        action="version", version=citation)
    parser.add_argument(
        "-info", help="show information about program and exit",
        action='version', version=info)
    parser.add_argument(
        "-debug", help="Keep Temporary files", action="store_true")
    parser.add_argument(
        "-verbose", help="Print on screen", action="store_true")
    parser.add_argument(
            "-expert", help="dont delete temp files", action="store_true")
    parser.add_argument(
        "-visual", help="MAke map of genomic islands " +
        "(requires perl and CGView)",
        action="store_true")
    parser.add_argument(
        "-seg", help="Provide segmentation threshold in range 0-1",
        type=float)
    parser.add_argument(
        "-annot",
        help="Annotate marker genes (Requires prodigal and Hmmer)",
        action="store_true")
...
def main():
    ...
    if verbose==1:
            ...
            try:
                    # open("gemini.out", "r")
                    os.system("./seg_clus %s %.15lf %.15lf %.15lf 1" %(outname, seg, clus1, clus2))
            except:
                    os.system("recseg_hieclus %s %.15lf %.15lf %.15lf 1" %(outname, seg, clus1, clus2))
    else:
            try:
                    os.system("./recseg_hieclus %s %.15lf %.15lf %.15lf 0" %(outname, seg, clus1, clus2))
            except:
                    os.system("recseg_hieclus %s %.15lf %.15lf %.15lf 0" %(outname, seg, clus1, clus2))
    ...
    max_clus=heapq.nlargest(3, range(len(size)), size.__getitem__)
    ...
    print("GI start end length Mosaic #genes", file=output)
    counter=0
    ...
    for i in range (len(put_gi_s)):
            for j in range (len(rna_start)):
                    ...
                    if put_gi_c[i] is not 'M':
                            put_gi_c[i]='-'
                    counter+=1
                    print(counter, put_gi_s[i], put_gi_e[i], put_gi_e[i]-put_gi_s[i], put_gi_c[i], put_gi_g[i], file=output)
